...
async initializeItems(): Promise <any> {
  const records =  await this.firestore.collection('Digital Record').valueChanges().pipe(first()).toPromise();
  return records;
}
async filterList(evt){
  ...
  this.records = this.records.filter(records =>{
    if (records && searchTerm){
      return(records.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1);
    }
  })
}
