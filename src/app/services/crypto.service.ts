...
export class CryptoService {
  ...
  getCrypto(id: string): Observable<Crypto> {
    return this.cryptoCollection.doc<Crypto>(id).valueChanges().pipe(
      take(1),
      map(crypto => {
        crypto.id = id;
        return crypto
  ...
  updateCrypto(crypto: Crypto): Promise<void> {
    return this.cryptoCollection.doc(crypto.id).update({ Name: crypto.Name, Surname: crypto.Surname, Address: crypto.Address , Phone_Number: crypto.Phone_Number , Id_passport: crypto.Id_passport , Proof_of_Residence: crypto.Proof_of_Residence , Deposite: crypto.Deposite});
  }
