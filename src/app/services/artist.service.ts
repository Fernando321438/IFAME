...
export class ArtistService {
  ...
  getArtist(id: string): Observable<Artist> {
    return this.artistCollection.doc<Artist>(id).valueChanges().pipe(
      take(1),
      map(artist => {
        artist.id = id;
        return artist
  ...
  updateArtist(artist: Artist): Promise<void> {
    return this.artistCollection.doc(artist.id).update({ Typeartist: artist.Typeartist, Name: artist.Name, Surname: artist.Surname , Alias: artist.Alias , Id_work: artist.Id_work , My_songs: artist.My_songs , Genres: artist.Genres, Royalties: artist.Royalties  });
  }
